...
module.exports = function (app, passport) {
            ...
            if (users) {
                ...
                userServices.create(req.body, token, null, (err, user) => {
                    if (err) {
                        ...
                        let userRegistrado = { email: "atencionalcliente@codegascolombia.com, directora.comercial@codegascolombia.com, servicioalcliente@codegascolombia.com" }
    ...
    */
    ...
    app.get('/x/v1/users/clientes', (req, res) => {
        if (req.session.usuario) {
            if (req.session.usuario.acceso == 'admin' || req.session.usuario.acceso == 'solucion' || req.session.usuario.acceso == 'comercial' || req.session.usuario.acceso == 'despacho' || req.session.usuario.acceso == 'adminTanque') {
    ...
    // FCM TOKEN ===========================
    // =====================================
    app.post('/x/v1/user/fcm-token', (req, res) => {
        ...
        userServices.updateFCMToken(userId, fcmToken, (err, result) => {
            if (err) {
                console.error('Error updating FCM token:', err);
                return res.json({
                    status: false,
                    code: 0,
                    message: 'Error actualizando token FCM'
                });
            }

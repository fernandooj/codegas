...
CREATE OR REPLACE FUNCTION get_vehiculos_con_pedidos_dia() RETURNS TABLE (
        vehiculo_id INT,
        placa VARCHAR,
        conductor_id INT,
...
BEGIN -- Obtener la fecha actual en la zona horaria de Colombia
fecha_actual := (NOW() AT TIME ZONE 'America/Bogota')::DATE;
RETURN QUERY WITH pedidos_asignados AS (
    SELECT p.*,
    ...
    WHERE p.eliminado = false
        AND p.carroId IS NOT NULL -- Filtrar por fechaEntrega = hoy (pedidos programados para hoy)
...
promedios_cliente AS (
        ...
        COALESCE(
            AVG(
                CASE
                    ...
                    AND TRIM(p.kilos) != ''
                    AND p.kilos ~ '^[0-9]+\.?[0-9]*$' THEN p.kilos::NUMERIC
                    ELSE NULL
                END
            ),
            0
    ...
    WHERE p.entregado = true
        ...
        AND TRIM(p.kilos) != ''
        AND p.kilos ~ '^[0-9]+\.?[0-9]*$' -- Nota: Se calculan promedios de todos los pedidos entregados con kilos válidos
...
pedidos_con_kilos AS (
    SELECT pd.*,
        ...
        CASE
            ...
            WHEN pd.cantidadkl IS NOT NULL
            AND pd.cantidadkl > 0 THEN pd.cantidadkl::NUMERIC -- Prioridad 2: parsear kilos si es válido
            WHEN pd.kilos IS NOT NULL
            AND TRIM(pd.kilos) != ''
            AND pd.kilos ~ '^[0-9]+\.?[0-9]*$' THEN pd.kilos::NUMERIC -- Prioridad 3: usar promedio del cliente si existe y es > 0
            WHEN pc.promedio_kilos IS NOT NULL
            AND pc.promedio_kilos > 0 THEN pc.promedio_kilos -- Prioridad 4: llamar a get_kilos_pedido como último recurso (puede ser lento pero asegura un valor)
...
vehiculos_con_pedidos AS (
    SELECT DISTINCT c._id as vehiculo_id,
        c.placa,
        c.capacidad,
...
SELECT v.vehiculo_id,
    ...
    COUNT(
        CASE
            WHEN p.entregado = true THEN 1
        END
    )::BIGINT as pedidos_entregados,
    COALESCE(
        jsonb_agg(
            jsonb_build_object(
                '_id',
            ...
            ORDER BY p.orden ASC NULLS LAST,
                p._id ASC
        ) FILTER (
